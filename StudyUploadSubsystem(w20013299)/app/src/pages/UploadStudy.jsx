...
const ErrorModal = ({ message, onClose }) => (
  <div style={{
    ...
    <button onClick={onClose} style={{ display: 'block', marginTop: '20px', marginRight: 'auto', marginLeft: 'auto', padding: '5px 20px', backgroundColor: '#007bff', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>
      Close
    </button>
  </div>
);
...
const UploadStudy = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.startDate < today || formData.endDate < today || formData.applicationDeadline < today) {
      setFormError('Start date, end date, and application deadline cannot be in the past.');
