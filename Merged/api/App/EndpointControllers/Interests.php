...
class Interests extends Endpoint
{
    ...
    private function getInterests($id)
    {
        ...
        $sql = "SELECT interestID FROM participant_interests WHERE userID = :id";
        $sqlParams = [':id' => $id];
    ...
    private function postInterests($id)
    {
        ...
        if (!is_numeric($interest_id) || $interest_id < 1 || $interest_id > 8) {
            throw new \App\ClientError(422);
        }
        $dbConn = new \App\Database(USERS_DATABASE);
        $sqlParams = [':id' => $id, 'interestID' => $interest_id];
        $sql = "SELECT * FROM participant_interests WHERE userID = :id AND interestID = :interestID";
        $data = $dbConn->executeSQL($sql, $sqlParams);
        if (count($data) === 0) {
            $sql = "INSERT INTO participant_interests (userID, interestID) VALUES (:id, :interestID)";
            $data = $dbConn->executeSQL($sql, $sqlParams);
        }
        return [];
    }
