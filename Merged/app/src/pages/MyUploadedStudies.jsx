...
const UploadedStudies = () => {
  ...
  useEffect(() => {
    ...
    const fetchStudies = async () => {
      setLoading(true);
      try {
        const response = await fetch(`https://w20012045.nuwebspace.co.uk/kv6002/merged/api/getstudiesbyclient?userId=${userId}`);
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch('https://w20012045.nuwebspace.co.uk/kv6002/merged/api/deletestudy', {
        method: 'POST',
        headers: {
