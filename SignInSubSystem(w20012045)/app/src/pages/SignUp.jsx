...
function SignUp() {
  ...
  const interests = [
    ...
    { id: 3, name: 'Psychology and Neuroscience' },
    { id: 4, name: 'Work and Education' },
    { id: 5, name: 'Health and Medicine' },
  ];
  ...
  const validateForm = () => {
    ...
    if (formData.usertypeID === 1 && selectedInterests.length === 0) {
      toast.error('Please select at least one interest.'); // interest validation for participants
      return false;
    }
          ...
          if (formData.usertypeID === 1) {
            ...
            Promise.all(interestPromises)
              ...
              .catch((error) => {
                ...
                toast.error('An error occurred while saving interests. Please try again later.');
              });
          } else {
            signIn(token);
            navigate('/');
                    ...
                    <li key={interest.id}>
                      <label>
                        <input
                          type="checkbox"
                          value={interest.id}
                          checked={selectedInterests.includes(interest.id.toString())}
